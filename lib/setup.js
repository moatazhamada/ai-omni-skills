...
`);
  ...
  // 9. Ask about periodic security scans (default: no)
  let scheduleSecurityScan = false;
  if (!dryRun && process.stdin.isTTY && process.stdout.isTTY) {
    const readline = await import('node:readline');
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise((resolve) => {
      rl.question('\n🔒 Periodic Security Scan\n   Run weekly security scans via cron?\n   This automatically checks your skills for vulnerabilities.\n   Schedule? [y/N] (default: no): ', resolve);
    });
    rl.close();
    scheduleSecurityScan = answer.trim().toLowerCase() === 'y' || answer.trim().toLowerCase() === 'yes';
  }

  if (scheduleSecurityScan) {
    console.log('\n📎 To schedule weekly scans, add this to your crontab:');
    console.log('   crontab -e');
    console.log('   # Weekly security scan (Sundays at 9:17 AM)');
    console.log('   17 9 * * 0 omni-skills security scan --no-llm');
    console.log('');
    console.log('   Or use your system scheduler (launchd, systemd, Task Scheduler)');
  }
  console.log('');

  // 10. Ask about remote backup (default: no)
  let remoteUrl = null;
  if (!dryRun && process.stdin.isTTY && process.stdout.isTTY) {
    const readline = await import('node:readline');
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
