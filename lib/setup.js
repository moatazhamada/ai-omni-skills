...
**Important**: Keep skills in the **private** repo (`skills-private/`). This protects them from:
...
The private repo is linked to `omni-skills` via `config.json`. If you don't have a private repo yet, the setup can create one.
...
');
  ...
  // 9. Ask about linking to private repo (default: no)
  let linkPrivateRepo = false;
  if (process.stdin.isTTY && process.stdout.isTTY) {
    const readline = await import('node:readline');
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise((resolve) => {
      rl.question('\n🔗 Private Skills Repository\n   Link an existing private repo for your skills?\n   (Skills stored only in the toolkit directory can be lost if reset)\n   Link? [y/N] (default: no): ', resolve);
    });
    rl.close();
    linkPrivateRepo = answer.trim().toLowerCase() === 'y' || answer.trim().toLowerCase() === 'yes';
  }
  ...
  if (linkPrivateRepo) {
    ...
    const repoUrl = await new Promise((resolve) => {
      rl.question('   Git URL (e.g. https://github.com/user/skills-private): ', resolve);
    });
    rl.close();
    if (repoUrl.trim()) {
      const { execSync } = await import('node:child_process');
      try {
        execSync(`git clone ${repoUrl.trim()} ${resolvedPrivate}`, { stdio: 'inherit' });
        console.log(`   ✓ Cloned private repo to ${resolvedPrivate}`);
      } catch {
        console.log(`   ⚠️ Could not clone. Please clone manually to ${resolvedPrivate}`);
      }
    }
  }
  console.log('');
