...
import { existsSync, mkdirSync, readFileSync, writeFileSync, lstatSync, readdirSync, copyFileSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { expandTilde } from './config.js';
import { AI_TOOL_DIRS, INSTRUCTION_PATTERNS, SKILL_DIR_NAMES } from './discover.js';
import { absPath } from './fs-utils.js';
...
const CONFIG_BACKUP_PATH = join(absPath('~/.config/skills'), 'config.json.bak');

function isInteractive() {
  return process.stdin.isTTY && process.stdout.isTTY && !process.env.CI;
}

export function findExistingSkillsDirectories(preferredToolkitDir = '~/Main/Projects/Skills') {
  ...
  const toolkitAbs = absPath(preferredToolkitDir);
  const toolkitParent = toolkitAbs ? dirname(toolkitAbs) : null;
  if (toolkitParent) {
    commonRoots.add(toolkitParent);
  }
  ...
  for (const root of commonRoots) {
    ...
    for (const entry of entries) {
      ...
      if (toolkitAbs && dirPath === toolkitAbs) continue;

      const sharedFile = join(dirPath, 'SHARED.md');
      const hasShared = existsSync(sharedFile);
      const skillFiles = findSkillFiles(dirPath);
      const matchesName = /skills/i.test(entry.name);

      if (hasShared || skillFiles.length > 0 || matchesName) {
        let reason;
        let score = 0;
        if (hasShared) {
          reason = 'SHARED.md found';
          score = 100 + skillFiles.length;
        } else if (skillFiles.length > 0) {
          reason = `${skillFiles.length} skill(s) found`;
          score = 50 + skillFiles.length;
        } else {
          reason = 'name contains "skills"';
          score = 10;
        }
        candidates.push({ path: dirPath, name: entry.name, reason, skillCount: skillFiles.length, score });
      }
    }
  }
...
export async function offerImportOrFresh({ publicPath, privatePath, toolkitDir, yes = false, dryRun = false } = {}) {
  const resolvedToolkit = toolkitDir || '~/Main/Projects/Skills';
  const defaultSkills = privatePath || publicPath || '~/my-skills';
  const canPrompt = isInteractive() && !yes;

  if (existsSync(CONFIG_BACKUP_PATH) && canPrompt) {
    let previous = {};
    try {
      previous = JSON.parse(readFileSync(CONFIG_BACKUP_PATH, 'utf8'));
    } catch {
      previous = {};
    }
    const previousSkills = previous.skillsPath || (previous.skillPaths ? previous.skillPaths[0] : null);
    const previousToolkit = previous.toolkitDir || resolvedToolkit;
    const display = previousSkills || previousToolkit;
    const answer = await promptUser(`Previous setup found at ${display}. Reuse? [Y/n/fresh] (default: Y): `);
    const normalized = answer.trim().toLowerCase();
    if (normalized === '' || normalized === 'y' || normalized === 'yes') {
      const configPath = join(absPath('~/.config/skills'), 'config.json');
      if (!dryRun) {
        try {
          copyFileSync(CONFIG_BACKUP_PATH, configPath);
        } catch {
          // ignore
        }
      }
      return {
        skillsPath: previousSkills || defaultSkills,
        toolkitDir: previousToolkit,
        source: 'backup',
        imported: true,
      };
    }
  ...
  const candidates = findExistingSkillsDirectories(resolvedToolkit);
  if (candidates.length > 0 && canPrompt) {
    console.log('\nExisting skills directories detected:');
    for (let i = 0; i < candidates.length; i++) {
      const c = candidates[i];
      console.log(`  ${i + 1}. ${c.path} (${c.reason})`);
    }
    const answer = await promptUser('Use [1-N], enter (c)ustom path, or start (f)resh? [f]: ');
    const normalized = answer.trim().toLowerCase();
    if (normalized === 'c' || normalized === 'custom') {
      const custom = await promptUser('Enter skills directory path: ');
      return { skillsPath: custom.trim() || defaultSkills, toolkitDir: resolvedToolkit, source: 'custom', imported: false };
    }
    const index = parseInt(normalized, 10);
    if (!isNaN(index) && index > 0 && index <= candidates.length) {
      return { skillsPath: candidates[index - 1].path, toolkitDir: resolvedToolkit, source: 'candidate', imported: false };
    }
  }
  ...
  return {
    ...
    source: privatePath || publicPath ? 'cli' : 'default',
    imported: false,
  };
}

export async function runSetup({ skillsPath, toolkitDir, dryRun = false } = {}) {
  console.log('=== Skills Toolkit Setup ===\n');
  if (dryRun) {
    console.log('--- DRY RUN ---');
    console.log('No files will be modified. Showing planned actions only.\n');
  }
  ...
  if (!existsSync(absSkills)) {
    if (dryRun) {
      console.log(`  [dry-run] would create ${resolvedSkills}`);
    } else {
  ...
  if (!existsSync(gitDir)) {
    if (dryRun) {
      console.log(`  [dry-run] would run git init in ${resolvedSkills}`);
    } else {
      try {
  ...
  // Set git config
  if (dryRun) {
    console.log('  [dry-run] would set git config user.email and user.name');
  } else {
    try {
  ...
  if (!existsSync(gitignorePath)) {
    if (dryRun) {
      console.log(`  [dry-run] would create ${resolvedSkills}/.gitignore`);
    } else {
  ...
  if (!existsSync(exampleSkillDir)) {
    if (dryRun) {
      console.log(`  [dry-run] would create example skill in ${resolvedSkills}/example-skill/`);
    } else {
  ...
  if (!existsSync(sharedPath)) {
    if (dryRun) {
      console.log(`  [dry-run] would create SHARED.md in ${resolvedSkills}/`);
    } else {
...
**DO NOT write SKILL.md files to ~/.claude/skills/, ~/.agents/skills/, ~/.kimi/skills/, etc.**
...
`);
  ...
  if (dryRun) {
    console.log('  [dry-run] would create initial git commit if there are changes');
  } else {
    try {
  ...
  // 8. Optional next steps (no prompts during setup)
  if (!dryRun) {
    console.log('\n--- Optional next steps ---');
    console.log('  Add a remote backup: cd ' + resolvedSkills + ' && git remote add origin <url>');
    console.log('  Scan skills for vulnerabilities: omni-skills manage security scan');
    console.log('');
  }
  ...
  if (!existsSync(configDir)) {
    if (dryRun) {
      console.log('  [dry-run] would create ~/.config/skills');
    } else {
  ...
  console.log(`Remote:       (none — add later with: cd ${resolvedSkills} && git remote add origin <url>)`);
  console.log('');
  ...
  if (suggestions.length > 0) {
    ...
    console.log('  2. Run `omni-skills manage classify <path>` to sort instruction files');
    console.log('  3. Run `omni-skills manage check --move` to rescue orphaned skills');
  }
  console.log('  Run `omni-skills manage index` to generate indexes');
  ...
  console.log('  Run `omni-skills manage security` to check security status');
