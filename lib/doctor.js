...
import { listTrash } from './fs-utils.js';
import { findOmniSkillsInstallations } from './install-detector.js';
...
export async function runDoctor(config) {
  ...
  // 2. Installation check (detect multiple/shadowed binaries)
  console.log('[installations]');
  const installations = findOmniSkillsInstallations();
  if (installations.length === 0) {
    issues.push('  omni-skills binary not found on PATH');
    ...
    issues.push(`  ${installations.length - 1} shadowed omni-skills installation(s) on PATH`);
    console.log('    Run `which -a omni-skills` to see all copies. Remove stale ones to avoid confusion.');
  }
  console.log('');

  // 3. Symlink check for instruction files
  ...
  // 7. Trash check
  console.log('[trash]');
  const trashedItems = listTrash();
  if (trashedItems.length === 0) {
    console.log('  ✓ Trash is empty');
  } else {
    console.log(`  ⚠ ${trashedItems.length} item(s) in trash`);
    console.log('    Run `omni-skills restore` to list them');
    console.log('    Run `omni-skills restore --prune=30` to remove items older than 30 days');
    if (trashedItems.length > 100) {
      issues.push(`  Trash contains ${trashedItems.length} items — consider pruning`);
    }
  }
  console.log('');

  // 8. Security check (SkillSpector)
