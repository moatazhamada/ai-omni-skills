import { existsSync, rmSync, unlinkSync, lstatSync, readFileSync, copyFileSync } from 'node:fs';
import { join } from 'node:path';
import readline from 'node:readline';
import { expandTilde } from './config.js';
import { getUpdateInstruction } from './version.js';
...
export async function uninstall(config) {
  ...
  if (existsSync(configPath)) {
    try {
      ...
      console.log('  ⚠️  Could not backup config.json');
    }
    try {
      rmSync(configPath);
  ...
  console.log(`  ${getUpdateInstruction()}`);
  console.log('  omni-skills setup');
  console.log('  omni-skills sync all');
  console.log('');
