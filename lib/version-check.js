...
export async function showUpdateStatus(options = {}) {
  ...
  if (cmp === 0) {
    if (!silent) {
      console.log(`✅ Omni Skills is up to date (v${current})`);
    }
  ...
  if (!silent) {
    ...
    console.log('');
    console.log('  Update:   npm install -g ai-omni-skills@latest');
  ...
  return { status: major ? 'major-behind' : minor ? 'minor-behind' : 'patch-behind', current, latest, major };
}
