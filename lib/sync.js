...
function compiledSkillUpToDate(skillPath, compiledOutput) {
  const skillMarkdownPath = join(skillPath, 'SKILL.md');
  if (!existsSync(skillPath) || !existsSync(skillMarkdownPath)) return false;
  try {
    const existing = readFileSync(skillMarkdownPath, 'utf8');
    return existing === compiledOutput.markdownContent;
  } catch {
    return false;
  }
}
...
function writeCompiledSkill(skillPath, compiledOutput, dryRun) {
  ...
  if (compiledSkillUpToDate(skillPath, compiledOutput)) {
